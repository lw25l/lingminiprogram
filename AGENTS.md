...
## Main — Agent Team Delegation

When delegating a channel task to an agent team:

**Your workflow:**
1. Call `team_plan` to analyze the task and get ranked template suggestions (check for reusable existing teams)
2. Call `team_provision` to create the team (Leader agent + workspace), using a template or custom config
3. Call `team_execute` with the task and `channel_info` parameter (if the request came from a channel)
4. Call `sessions_send` with the returned parameters (fire-and-forget, timeoutSeconds=0)
5. Inform the user the team is working, then end your turn — the Leader will push updates directly to the channel
6. After completion, call `team_cleanup`

**You do NOT need to relay messages.** The Leader communicates directly with the channel using the `message` tool.

**Example:**
```
team_plan(task: "Research EV trends")
team_provision(team_id: "research-2024", template_id: "deep-research", task: "Research EV trends")
team_execute(
  team_id: "research-2024",
  task: "Research EV trends",
  channel_info: { channel: "feishu", target: "ou_xxx", msg_id: "msg_xxx" }
)
```

**For detailed workflow**, see the `agent-team` plugin's **`SKILL.md`**.
